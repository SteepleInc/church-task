...
Generic `status`, `inactivated_at`, and `inactivated_by` fields are not default base fields. Table-specific lifecycle fields are added only when Church Work domain language requires them. Database foreign key constraints are not used by default; relationship cleanup is enforced in Zero mutators, Effect/Drizzle services, and tests. Unique indexes remain important for domain invariants.
...
Better Auth uses Postgres/Drizzle. Auth-owned ephemeral tables stay close to Better Auth's expected schema. Church Work copies the PreachX onboarding/session plugin pattern while preserving the current Church Work onboarding semantics.
