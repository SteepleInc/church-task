...
Onboarding creates the Church the moment the Church Profile step is submitted (`organization.create` + `setActive`, with the organization creation hook seeding the default Workflow, Key Dates, and Starter Teams), rather than batching all persistence into a final "complete" call. The current step is derived — never stored — from URL search state plus Active Church state via a `resolveStep` function, ported from PreachX's onboarding data flow, whose protections (atomic `completeOnboarding` plugin, `clearOrgForOnboarding`, completed-flag gating) were battle-tested there. Steps are `churchProfile` -> `initialTeams` -> `finished`; the Finished step remains a stable completion/redirect landing step and does not imply billing is implemented. The Initial Teams step is a live editor over persisted teams, not a local draft list.
...
- Deliberate deviations from the PreachX reference: no `orgId` step param and a stronger no-active-org rule in `resolveStep` (a stale `?step=` URL can never render an orgless step). We keep PreachX's session read model for auth routing: `org_completed_onboarding`, `org_role`, and `user_role` are copied onto the session when the Active Church changes, when Onboarding completes, and when a new session is created. Route guards can decide quickly from the session, while pages read live Church/product state through Zero or bounded server APIs.
