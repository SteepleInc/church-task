...
- Deliberate deviations from the PreachX reference: no `orgId` step param, a stronger no-active-org rule in `resolveStep` (a stale `?step=` URL can never render an orgless step), and no session-mirror flag — gating reads the reactive Convex `getActiveOrganization` query as the single source of truth, since Convex reactivity removes the stale-session race the mirror existed to solve.
