...
export const startZeroCacheHarness = async (options: {
  ...
  const child = spawn(process.execPath, [getZeroCacheCliPath()], {
    env: {
      ...
      ZERO_LOG_LEVEL: "warn",
      ZERO_MUTATE_ALLOWED_CLIENT_HEADERS: "x-church-task-session-id,x-church-task-user-id",
      ZERO_MUTATE_URL: `${options.apiBaseUrl}/api/zero/mutate`,
      ZERO_NUM_SYNC_WORKERS: "1",
      ZERO_PORT: String(port),
      ZERO_QUERY_ALLOWED_CLIENT_HEADERS: "x-church-task-session-id,x-church-task-user-id",
