...
describe("create task quick action", () => {
  ...
  test("frames the dialog as a Subtask when opened from a comment with a parent", () => {
    ...
    expect(source).toContain("const isCreatingSubtask = Boolean(state?.parentTaskLabel)");
    expect(source).toContain('isCreatingSubtask ? "New Subtask" : "New Task"');
    expect(source).toContain('isCreatingSubtask ? "Create Subtask" : "Create Task"');
